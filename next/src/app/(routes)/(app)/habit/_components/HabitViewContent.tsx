...
const HabitViewContent = ({ habits, totalHabitCount, todayDoneHabitCount, todayDoneHabitRate }: Props) => {
  return (
    <div className="w-full desktop:grid desktop:grid-cols-[minmax(0,11fr)_minmax(0,9fr)] desktop:items-start desktop:gap-x-8">
      {totalHabitCount > 0 && (
        <aside className="mb-6 desktop:sticky desktop:top-[calc(var(--page-toolbar-height,68px)+24px)] desktop:col-start-2 desktop:row-start-1 desktop:mb-0 desktop:border-l desktop:border-theme-border/60 desktop:pl-8">
          <HabitTodayProgress
