...
const HabitInfoHeader = ({ habitData }: Props) => {
  return (
    <section className="flex w-full flex-col items-center gap-3 pb-1">
      <HabitIcon iconKey={habitData?.iconKey} className="text-[56px]" />
      <StarRating maxRating={3} rating={(habitData?.priority ?? 0) + 1} className="items-center justify-center !text-2xl" />
