...
const DiaryCardShell = forwardRef<HTMLDivElement, DiaryCardShellProps>(
  ({ variant = 'large', className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "box-border w-full overflow-hidden rounded-theme bg-theme-surface shadow-theme-section backdrop-blur-xl",
        variant === 'small' && "h-[170px]",
        variant === 'large' && "flex h-auto min-h-[250px] shrink-0 flex-col items-start justify-between max-tablet:min-h-[200px]",
        className,
      )}
      {...props}
    />
